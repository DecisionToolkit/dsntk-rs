...
#[test]
fn _0005() {
  let node = AstNode::Path(Box::new(AstNode::Path(Box::new(AstNode::Name("a".into())), Box::new(AstNode::Name("b".into())))), Box::new(AstNode::Name("days".into())));
...
#[test]
fn _0006() {
  let node = AstNode::Path(Box::new(AstNode::Name("a".into())), Box::new(AstNode::Path(Box::new(AstNode::Boolean(false)), Box::new(AstNode::Name("days".into())))));
  let scope = te_scope(r#"{ a: [{ b: @"P1DT5H" }, { b: @"P2DT6H" }, { b: @"P3DT7H" }] }"#);
  assert_eq!(r#"null(expected name node, actual node is: Path(Boolean(false), Name(Name("days"))))"#, crate::evaluate(&scope, &node).to_string());
}
