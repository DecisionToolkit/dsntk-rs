...
fn _0004() {
  ...
  assert_eq!(r#"null(build_path: no context in list)"#, crate::evaluate(&scope, &node).to_string());
}
...
fn _0006() {
  ...
  let scope = te_scope(r#"{ a: [{ b: @"P1DT5H" }, { b: @"P2DT6H" }, { b: @"P3DT7H" }] }"#);
  assert_eq!(
    r#"null(invalid qualified name node: Path(Boolean(false), Name(Name("days"))))"#,
...
fn _0007() {
  ...
  let scope = te_scope(r#"{ a: [{ b: @"P1DT5H" }, { b: @"P2DT6H" }, { b: @"P3DT7H" }] }"#);
  assert_eq!(r#"null(invalid qualified name node: Boolean(false))"#, crate::evaluate(&scope, &node).to_string());
}
