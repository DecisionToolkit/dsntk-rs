...
macro_rules! iter {
  ($b:tt, $benchmark:expr) => {
    if DRY_RUN {
      ...
      BENCHMARK_COUNT.fetch_add(1, Ordering::SeqCst);
      println!("total benchmark count: {}", BENCHMARK_COUNT.load(Ordering::SeqCst));
    } else {
      $b.iter(|| $benchmark);
    }
  };
...
use {from_examples, iter, model_evaluator_from_examples, model_name_from_examples, model_namespace_from_examples};
...
fn assert_decision_service(model_evaluator: &ModelEvaluator, model_namespace: &str, model_name: &str, invocable_name: &str, input: &str, expected: &str) {
  let actual = model_evaluator.evaluate_invocable(model_namespace, model_name, invocable_name, &context(input)).to_string();
  assert_eq!(
    expected, actual,
